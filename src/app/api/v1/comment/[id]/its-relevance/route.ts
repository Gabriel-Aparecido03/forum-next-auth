...
export async function POST(_: Request, { params }: { params: { id: string } }) {
  ...
  const hasAlready = await prisma.commentRelevant.findFirst({
    where : {
      profile_id,
  ...
  const hasAlreadyMakedAsNotRelevant = await prisma.commentNotRelevant.findFirst({
    where : {
      profile_id ,
      comment_id : comment.id,
    }
  ...
  if(hasAlreadyMakedAsNotRelevant) await prisma.commentNotRelevant.delete({
    where : {
  ...
  if(hasAlready) {
    await prisma.commentRelevant.deleteMany({
      where : {
        profile_id,
        comment_id : comment.id,
      }
  ...
  await prisma.commentRelevant.create({
    data : {
      profile_id,
      comment_id : comment.id,
    }
