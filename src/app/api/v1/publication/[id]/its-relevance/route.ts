...
export async function POST(_: Request, { params }: { params: { id: string } }) {
  ...
  const hasAlready = await prisma.publicationRelevant.findFirst({
    where : {
      profile_id,
  ...
  const hasAlreadyMakedAsNotRelevant = await prisma.publicationNotRelevant.findFirst({
    where : {
      profile_id ,
      publication_id : publication.id,
    }
  ...
  if(hasAlreadyMakedAsNotRelevant) await prisma.publicationNotRelevant.delete({
    where : {
      id : hasAlreadyMakedAsNotRelevant.id
    }
  })

  if(hasAlready) {
    await prisma.publicationRelevant.deleteMany({
      where : {
        profile_id,
        publication_id : publication.id,
      }
  ...
  await prisma.publicationRelevant.create({
    data : {
      profile_id,
      publication_id : publication.id,
    }
